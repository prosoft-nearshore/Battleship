	...
	try {
		...
		for (var myMoveIdx = 0; myMoveIdx < myMoves.length; myMoveIdx++) {
			...
			if (myMove.isHit && !myMove.isSunk) {
				...
				if (fireboat.myPreviousLastMove != null && fireboat.myLastMove != null) {
				  if (fireboat.myPreviousLastMove.x == fireboat.myLastMove.x) {
				    var yDif = fireboat.myLastMove.y - fireboat.myPreviousLastMove.y;
				    var yIdx;
				    if (yDif > 0) {
				      yIdx = myMove.y + 1;
				      if (yIdx < 10)
				        board[myMove.x][yIdx] += 1;
				    } else {
				      yIdx = myMove.y - 1;
				      if (yIdx < 10)
				        board[myMove.x][yIdx] += 1;
				    ...
				    var xDif = fireboat.myLastMove.x - fireboat.myPreviousLastMove.x;
				    var xIdx;
				    if (xDif > 0) {
				      xIdx = myMove.x + 1;
				      if (xIdx < 10)
				        board[xIdx][myMove.y] += 1;
				    } else {
				      xIdx = myMove.x - 1;
				      if (yIdx < 10)
				        board[xIdx][myMove.y] += 1;
		...
		if (fireboat.myLastMove != null) {
		  fireboat.myPreviousLastMove = {
		    x: fireboat.myLastMove.x,
		    y: fireboat.myLastMove.y
		  };
		}
